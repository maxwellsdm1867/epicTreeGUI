...
must_haves:
  truths:
    ...
    - "Test suite validates one-time mask building (no centralized mask property)"
  artifacts:
    ...
    - path: "tests/test_ugm_persistence.m"
      provides: "UGM file persistence tests with one-time mask building validation"
      contains: "testSaveAndLoadRoundTrip, testEpochCountMismatchWarns, testFindLatestUGM, testConstructorLoadOptions, testOnTimeMaskBuilding"
  key_links:
    ...
    - from: "tests/test_ugm_persistence.m"
      ...
      via: "Save/load round-trips with temp directory, validates one-time mask building"
---

<objective>
Write comprehensive tests validating selection state filtering and .ugm persistence with simplified architecture (one-time mask building), ensuring BUG-001 is fixed and the three-file architecture works correctly.

Purpose: Phase 0.1 success criteria #4 requires "Tests validate selection filtering works end-to-end." These tests serve as regression guards ensuring the critical selection workflow never breaks again, and that .ugm file persistence works reliably with the simplified architecture (build mask on save, copy mask on load, no real-time sync).

Output: Two test files covering selection state management and .ugm file persistence with validation of one-time mask building.

Note: This plan depends on both Plan 01 (epicTreeTools methods) and Plan 02 (GUI integration). The tests focus on the epicTreeTools API; GUI testing (button clicks, dialogs, close handler) is deferred to manual verification since MATLAB's unittest framework does not easily support interactive GUI testing.
</objective>
...
<task type="auto">
  <name>Task 2: Write .ugm persistence tests with one-time mask building validation</name>
...
14. **testOneTimeMaskBuilding** - Validate that saveUserMetadata builds mask from isSelected flags (not from a centralized property). Deselect epochs, save, verify .ugm contains correct mask. Verify tree does NOT have a selectionMask property (validates simplified architecture).

15. **testCommandWindowWarnings** - Capture command window output during save and load operations. Verify save prints selection count, load prints excluded count. Use `evalc()` to capture fprintf output.
...
- For test 14, use `isprop(tree, 'selectionMask')` to verify no centralized mask property exists.
- For test 15, use `output = evalc('tree.saveUserMetadata(filepath)')` to capture command window output, then verify it contains expected strings like "Saved selection mask" and epoch counts.
  </action>
  <verify>
Run the test file using MCP MATLAB:
```matlab
...
All tests should pass (or skip gracefully if test data unavailable).
  ...
  <done>
- 15 tests covering .ugm save, load, find, generate, constructor options, and one-time mask building
...
- One-time mask building test validates simplified architecture (no centralized mask property)
- Command window warnings test validates user-facing messages
- All tests pass with real data or skip gracefully if data unavailable
  </done>
</task>
...
5. testOneTimeMaskBuilding validates no centralized selectionMask property exists
6. testCommandWindowWarnings validates save and load print expected messages
</verification>

<success_criteria>
- Selection state tests prove BUG-001 is fixed with 10 test cases
- .ugm persistence tests prove three-file architecture works with 15 test cases
- One-time mask building test validates simplified architecture (build on save, copy on load, no sync)
- Command window warnings test validates user-facing epoch count messages
