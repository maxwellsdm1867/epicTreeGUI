...
This ensures the GUI knows where the .mat file lives so it can discover/create .ugm files in the same directory. Since Plan 01 updated `loadEpicTreeData()` to store `source_file` in the data struct and the epicTreeTools constructor reads it into `sourceFile`, this flow is now automatic: `loadEpicTreeData` -> `data.source_file` -> `tree.sourceFile` -> `gui.matFilePath`.
